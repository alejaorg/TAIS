...
bool resuelveCaso() {
	...
	vector<tNodo> g(M, { 0, false });
	int res = -1, nOpers;
	cin >> nOpers;
	vector<pair<int, int>> opers(nOpers);

	for (pair<int, int> & a : opers) //first = a // second = b
		cin >> a.first >> a.second;
		
	//CaminoMasCorto camino(g, S, T, opers, res);
	queue<int> q;
	g[S].dist = 0; g[S].visit = true;
	q.push(S);
	...
	while (!q.empty() && res == -1) {
		...
		if (aux == T)
			...
			for (pair<int, int> w : opers) {
				aux = (v * w.first + w.second) % M;
				if (!g[aux].visit) {
					g[aux].dist = g[v].dist + 1; g[aux].visit = true;
					q.push(aux);
				}
			}
		}
